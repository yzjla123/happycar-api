...
public class DESCodec {
	public static final String keys = "ucarucar";
    //算法名称  
    ...
    public static String encrypt(String data) throws Exception {  
        Key k = toKey(Base64.decodeBase64(initkey()));                           //还原密钥  
        Cipher cipher = Cipher.getInstance(CIPHER_ALGORITHM);              //实例化Cipher对象，它用于完成实际的加密操作  
        cipher.init(Cipher.ENCRYPT_MODE, k);                               //初始化Cipher对象，设置为加密模式  
        return Base64.encodeBase64URLSafeString(cipher.doFinal(data.getBytes())); //执行加密操作。加密后的结果通常都会用Base64编码进行传输  
    }  
    ...
    public static String decrypt(String data) throws Exception {  
        Key k = toKey(Base64.decodeBase64(initkey()));  
    ...
    public static void main(String[] args) throws Exception {  
        ...
        System.out.println(URLEncoder.encode("oX7HPg4/qgrL3UJkrRxXsCWPnooLrh/iw76rlXhxtCI="));
    }  
}  
